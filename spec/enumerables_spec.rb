...
describe "Enumerable" do
  ...
  describe "#my_each" do
    it 'my_each must be an Enumerator ' do
      expect([1, 2, 3].my_each.class == Enumerator).to eq(true)
    end
  ...
  describe '#my_each_index' do
    it 'my_each_with_index must be an Enumerator ' do
      expect([1, 2, 3].my_each_with_index.class == Enumerator).to eq(true)
    end
    it 'my_each_with_index when a block is given when self is a range' do
      concat1 = ''
      (1..4).each_with_index(&proc { |n, i| concat1 += 'n: ' + n.to_s + ', i: ' + i.to_s + '+++' })
      concat2 = ''
  ...
  describe '#my_select' do
    it 'returns an enumerator if no block is given' do
      expect([1, 2, 3, 4].my_select.class == Enumerator).not_to eq(false)
    end
  ...
  describe '#my_all?' do
    it 'when a Regex is passed as an argument returns true if all of the collection matches the Regex ' do
      expect(['car', 'cat'].all?(/a/) == ['car', 'cat'].my_all?(/a/)).not_to eq(false)
  ...
  describe '#my_any?' do
    ...
    it 'when an array is given with a block, it should perform the function on every element of the array' do
      expect([2, 3, 4, 4, 4, 3, 2, 2, 1].my_any? { |ele| ele.odd? }).to eq(true)
    end
  end
  describe '#my_none? ' do
    it 'returns false if the block ever returns true for all elements::range' do
      expect((1..3).none?(&proc { |num| num % 2 == 0 }) == (1..3).my_none?(&proc { |num| num % 2 == 0 })).to eq(true)
  ...
  describe '#my_map' do
    it 'should returns an Enumerator if no block is given' do
      expect([1, 2, 3].map.class == [1, 2, 3].my_map.class).to eq(true)
    end
    it 'when an array is given with a block, it should perform the function on every element of the array ' do
  ...
  describe '#my_inject' do
    it 'should searches for the longest word in an array of strings ' do
      expect(['to', 'four', 'tri'].my_inject(&proc { |a, b| [a, b].max_by { |m| m.length } }) ==
               ['to', 'four', 'tri'].inject(&proc { |a, b| [a, b].max_by { |m| m.length } })).to eq(true )
    end
    it 'when a block is given without an initail value combines all elements of enum by applying a binary operation, specified by a block::range' do
      expect((1..3).inject(&proc{|total, num| total*num}) == (1..3).my_inject(&proc{|total, num| total*num})).not_to eq(false )
    end
  end
end
